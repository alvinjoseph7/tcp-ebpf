...
// int bpf_iw(struct sock *skops)
{
	...
	int iw = 40;
	int rv = 0;		// stores retval and also taken as index for map
	struct in_addr addr;
	int key = 0;
	int val = 0;
	// int* user_wnd;
	...
	// int* user_wnd = bpf_map_lookup_elem(&wnd_map, &key);
	/* if (!user_wnd) {
		rv = bpf_map_update_elem(&wnd_map, &key, &val, 0);
		// if (rv < 0)
	...
	case BPF_SOCK_OPS_ACTIVE_ESTABLISHED_CB:
    	...
    	// inet_ntop(AF_INET, &addr, ip_human_readable, INET_ADDRSTRLEN);

		rv = bpf_setsockopt(skops, SOL_TCP, TCP_BPF_IW, &iw, sizeof(iw));
		bpf_printk("New initial cwnd = %d, IP: %pI4", iw, &addr.s_addr);
		// if (*user_wnd >= 0) {
		// 	// rv = bpf_setsockopt(skops, SOL_TCP, TCP_BPF_IW, user_wnd, sizeof(*user_wnd));
		// 	bpf_printk("Init cwnd %d, IP: %pI4", *user_wnd, &addr.s_addr);

		// }

		break;
	// case BPF_SOCK_OPS_PASSIVE_ESTABLISHED_CB:q
	// iw = 20;
